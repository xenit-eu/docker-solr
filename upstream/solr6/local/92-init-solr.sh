...
function makeConfigs {
  ...
  if [ $ALFRESCO_SOLR_SUGGESTER_ENABLED = true ]; then
  ...
  for coreName in "${DEFAULT_CORES[@]}"; do
    newCore=${SOLR_DIR_ROOT}/$coreName
    CONFIG_FILE_CORE=$newCore/conf/solrcore.properties

    if [ $coreName = alfresco ]; then
      for coreAlfrescoName in "${DEFAULT_CORES_ALFRESCO[@]}"; do
        # only use a collection in the case of a real sharded setup
        if [ $coreAlfrescoName != $coreName ]; then
        ...
        # SOLR_BACKUP_DIR only useful for ASS v2.0.6* and up
        if [ $coreAlfrescoName != $coreName ]; then
        ...
        if [ $ALFRESCO_SOLR_FACETABLE_CATEGORIES_ENABLED = true ]; then
...
if [ $ALFRESCO_SSL != none ] && [ $ALFRESCO_SSL != secret ]; then
  ...
  if [ -f "$SECURITY_FILE" ]; then
    cat $SECURITY_FILE | jq 'del(.authentication)' >$SECURITY_FILE
